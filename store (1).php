...
$sql = "SELECT pic, name,  map, phone, time,tags FROM tgos";
			...
			<?php
				...
				if ($result->num_rows > 0) {
					while($row = $result->fetch_assoc()) {
						...
						echo "<td data-label='name'><a href='introduce.php?id=" . urlencode($row["id"]) . "'>" . htmlspecialchars($row["name"]) . "</a></td>";
						echo "<td data-label='map'><a href='https://www.google.com/maps/search/?api=1&query=" . urlencode($row["map"]) . "' target='_blank'>" . htmlspecialchars($row["map"]) . "</a></td>";
						echo "<td data-label='phone'>" . htmlspecialchars($row["phone"]) . "</td>";
						echo "<td data-label='time'>" . htmlspecialchars($row["time"]) . "</td>";
						echo "<td><div class='tags-container'>";

						$tags = explode(',', $row['tags']); // 假設標籤以逗號分隔
						foreach ($tags as $tag) {
							$tag = trim($tag);
							if (array_key_exists($tag, $tagPages)) {
								echo "<a href='" . htmlspecialchars($tagPages[$tag]) . "' class='tag-btn'>" . htmlspecialchars($tag) . "</a>";
							} else {
